...
static int32_t process_app_settings_page( const char* url_parameters, const char* url_query_string, wiced_http_response_stream_t* stream, void* arg, wiced_http_message_body_t* http_data )
{
    ...
    wiced_http_response_stream_write_resource( stream, &resources_apps_DIR_secure_mqtt_DIR_aws_config_html );
...
#else
        wiced_http_server_start(http_server, HTTP_PORT, 4, config_http_page_database, WICED_CONFIG_INTERFACE, DEFAULT_URL_PROCESSOR_STACK_SIZE );
#endif
